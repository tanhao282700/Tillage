...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getHeadData(){
        ...
        utils.post('power/history/table/header',objP).then(res=>{
          ...
          if(res.code==0){
            ...
            this.getReportData('1207',this.date_s,this.date_e,this.search_cont,1);//默认获取当前时间前一个小时的数据
      ...
      //获取数据报表
      getReportData(selfID='1207',date_s=utils.time(new Date()/1000-1*60*60,4),date_e=utils.time(new Date()/1000,4),search_cont='',num=1){
        this.loading = true;
        let objP = {
          'sys_menu_id':'3',
          'self_id':selfID,
      ...
      changeQuick(id){
        ...
        if (id == 'day') {
          ...
          this.table.page = 1;
          this.getReportData('1207', (utils.time(new Date() / 1000, 1) + ' 00:00:00'), utils.time(new Date() / 1000, 4), this.search_cont, 1);//获取今日到当前时间的数据
        } else if (id == 'mon'){

          this.date_s = (utils.time(new Date() / 1000, 9) + '-01 00:00:00');
          this.date_e = utils.time(new Date() / 1000,4);
          this.table.page = 1;
          this.getReportData('1207', (utils.time(new Date() / 1000, 9) + '-01 00:00:00'), utils.time(new Date() / 1000, 4), this.search_cont, 1);//获取本月到当前时间的数据
        }
      },
      //分页：切换页面
      changePage(val){
        this.num = val;
        this.getReportData('1207',this.date_s,this.date_e,this.search_cont,val);
        //console.log(val)
      },
      //查询按钮事件
      searchData(){
        ...
        this.table.page = 1;
        this.getReportData('1207', this.date_s, this.date_e, this.search_cont, 1);//获取特定时间的数据
      }
    },
    created() {
